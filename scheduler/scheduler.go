...
type Scheduler interface {
	...
	Registered(driver SchedulerDriver, frameworkID *mesos.FrameworkID,
	...
	Reregistered(driver SchedulerDriver, masterInfo *mesos.MasterInfo)

	// Invoked when the scheduler becomes "disconnected" from the master
	// (e.g., the master fails and another is taking over).
	Disconnected(driver SchedulerDriver)
	...
	ResourceOffers(driver SchedulerDriver, offers []*mesos.Offer)
	...
	OfferRescinded(driver SchedulerDriver, offerID *mesos.OfferID)
	...
	StatusUpdate(driver SchedulerDriver, status *mesos.TaskStatus)

	// Invoked when an executor sends a message. These messages are best
	// effort; do not expect a framework message to be retransmitted in
	// any reliable fashion.
	FrameworkMessage(driver SchedulerDriver, executorID *mesos.ExecutorID,
		slaveId *mesos.SlaveID, data string)

	// Invoked when a slave has been determined unreachable (e.g.,
	// machine failure, network partition). Most frameworks will need to
	// reschedule any tasks launched on this slave on a new slave.
	SlaveLost(driver SchedulerDriver, slaveID *mesos.SlaveID)

	// Invoked when an executor has exited/terminated. Note that any
	// tasks running will have TASK_LOST status updates automagically
	// generated.
	ExecutorLost(driver SchedulerDriver, executorID *mesos.ExecutorID,
		slaveID *mesos.SlaveID, status int)

	// Invoked when there is an unrecoverable error in the scheduler or
	// scheduler driver. The driver will be aborted BEFORE invoking this
	// callback.
	Error(driver SchedulerDriver, message string)
}
...
func NewMesosSchedulerDriver(sched Scheduler, framework *mesos.FrameworkInfo,
	master string, credential *mesos.Credential) *MesosSchedulerDriver {
